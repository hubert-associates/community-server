...
const STORAGE_KEY = 'setupIsFinished';
...
export class SetupHttpHandler extends BaseHttpHandler {
  ...
  public async canHandle(): Promise<void> {
    // Check the storage to see if the boolean was set to false due to server restart
    if (!this.finished) {
      this.finished = Boolean(await this.storage.get(STORAGE_KEY));
    }

    if (this.finished) {
      throw new MethodNotAllowedHttpError('Setup is finished.');
    }
  }
